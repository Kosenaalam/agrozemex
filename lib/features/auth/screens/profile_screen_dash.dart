// F:\agrozemex\lib\features\auth\screens\profile_screen.dart (rename if needed)
import 'package:agrozemex/features/auth/screens/login_screen.dart'; // NEW: For logout redirect
      ...
      builder: (context, snapshot) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            actions: [
              IconButton( // Logout button (already there)
              
                icon: const Icon(Icons.logout),
                color: Color(0xffffffff),
                onPressed: () async {
                  await auth.logout();
                  Navigator.pushReplacement( // FIXED: Replace with Login
                    context,
...
const SizedBox(height: 20),
                ...
                //--------------
                const SizedBox(height: 20),
                if (role == 'seller') // Only show if seller
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                ...
                if (role == 'buyer') // NEW: Show for buyer (no dashboard)
                  const Center(
